...
class PostgresUploader:
    def __init__(
        ...
        db: str = "business",
        user: str = "saveliy",
        pswd: str = os.getenv("PSQL_BUSINESS_PSWD"),
        host: str = "54.226.115.222",
        port: int = 5432,
    ):
        self.parm = {
            "dbname": db,
    ...
    def convert_to_geom(self, point):
        return f"POINT({point[0]}, {point[1]})"

    def connect(self):
        return psycopg2.connect(**self.parm)

    def mogrify(sel, lines):
        with self.connect() as connection:
            with connection.cursor() as cursor:
                args_list = b",".join(
                    cursor.mogrify("(%s)", (self.convert_to_geom(x))) for x in lines
                )
        return args_list
    ...
    #         + args_str
    #         + b" ON CONFLICT (location) DO NOTHING"
    #     )
    #     self.execute(query)
...
if __name__ == "__main__":
    ...
    ss.sparkContext.broadcast(category_dict)

    sqlContext = SQLContext(ss)
    dataframe = ss.read.csv(
        "s3a://saveliy.de.project/All_US_STATES/A*.csv",
        encoding="windows-1252",
        header=True,
    )
    # step 0 remove extra columns
    df = dataframe.repartition(180).drop(*columns_drop)
